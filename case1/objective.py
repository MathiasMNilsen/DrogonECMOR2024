...
def co2_per_toe(pred_data, keys_opt, report, save_emissions=False, save_npv=False):
    '''
    Parameters
    ----------
    ...
    with Pool(keys_opt['parallel']) as pool:
        values = pool.map(_obj, indecies)

    toe_values = [val[0] for val in values]
    npv_values = [val[1] for val in values]
    emissions  = [val[2] for val in values]
    

    if save_emissions:
        emissions = np.array(emissions).squeeze()
        np.save('co2_emissions', emissions)

    if save_npv:
        npv_values = np.array(npv_values).squeeze()
        np.save('npv', npv_values)
    ...
    return np.asarray(toe_values)/scaling
